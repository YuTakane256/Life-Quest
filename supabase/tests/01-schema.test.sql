...
select ok(not has_function_privilege('authenticated', 'public.complete_task_apply(uuid, uuid, integer, date, jsonb, jsonb, text)', 'execute'),
    'authenticated cannot execute complete_task_apply');
select ok(has_function_privilege('service_role', 'public.complete_task_apply(uuid, uuid, integer, date, jsonb, jsonb, text)', 'execute'),
