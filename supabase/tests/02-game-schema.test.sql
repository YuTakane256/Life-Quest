...
select ok(not has_function_privilege('authenticated', fn, 'execute'), 'authenticated cannot execute ' || fn)
  from unnest(array[
    'public.complete_subtask_apply(uuid, uuid, integer, date, jsonb, integer, jsonb, text)',
...
select ok(has_function_privilege('service_role', 'public.complete_subtask_apply(uuid, uuid, integer, date, jsonb, integer, jsonb, text)', 'execute'),
